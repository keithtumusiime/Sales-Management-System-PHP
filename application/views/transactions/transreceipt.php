...
<div id="transReceiptToPrint">
    <div class="row">
        <div class="col-xs-12 text-center text-uppercase">
            ...
            <b>Everce Kyomuhendo Stationaries</b>
            <div>+256 770786064, +256 781363816<br>evercekyomuhendo@gmail.com</div>
        </div>
    </div>
	...
	<div class="row" style='font-weight:bold'>
		...
		<div class="col-xs-4">Total(UGX)</div>
	</div>
    ...
    <div class="row">
        <div class="col-xs-12 text-right">
            <b>Total: UGX <?=isset($init_total) ? number_format($init_total, 2) : 0?></b>
        </div>
    </div>
    <hr style='margin-top:2px; margin-bottom:0px'>      
    <div class="row">
        <div class="col-xs-12 text-right">
            <b>Discount(<?=$discountPercentage?>%): UGX <?=isset($discountAmount) ? number_format($discountAmount, 2) : 0?></b>
        </div>
    </div>       
    <div class="row">
        <div class="col-xs-12 text-right">
            <?php if($vatPercentage > 0): ?>
            <b>VAT(<?=$vatPercentage?>%): UGX <?=isset($vatAmount) ? number_format($vatAmount, 2) : ""?></b>
            <?php else: ?>
            VAT inclusive
            <?php endif; ?>
        </div>
    ...
    <div class="row">
        <div class="col-xs-12 text-right">
            <b>FINAL TOTAL: UGX <?=isset($cumAmount) ? number_format($cumAmount, 2) : ""?></b>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-xs-12">
            <b>Amount Tendered: UGX <?=isset($amountTendered) ? number_format($amountTendered, 2) : ""?></b>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12">
            <b>Change: UGX <?=isset($changeDue) ? number_format($changeDue, 2) : ""?></b>
        </div>
    </div>
    <hr style='margin-top:5px; margin-bottom:0px'>
    <div class="row margin-top-5">
